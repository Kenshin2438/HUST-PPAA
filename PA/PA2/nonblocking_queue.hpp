...
template <typename T, std::size_t MAXN = 2048U>
  requires(std::is_trivial_v<T> && MAXN >= 2 && (MAXN & (MAXN - 1)) == 0)
class NONBLOCKING_QUEUE {
 ...
 private:
  static constexpr std::size_t MOD_MASK{MAXN - 1};

 public:
  constexpr NONBLOCKING_QUEUE() noexcept {
    for (std::size_t i = 0; i < MAXN; i++) buffer_[i].version.store(i);
  }

  constexpr void push(const T& value) noexcept {
    std::size_t tail = tail_.load(std::memory_order::relaxed), index;
    while (true) {
      index = tail & MOD_MASK;
      if (buffer_[index].version.load(std::memory_order::acquire) == tail) {
        if (tail_.compare_exchange_weak(tail, tail + 1U, std::memory_order::relaxed)) break;
      } else {
  ...
  constexpr void pop(T& value) noexcept {
    std::size_t head = head_.load(std::memory_order::relaxed), index;
    while (true) {
      index = head & MOD_MASK;
      if (buffer_[index].version.load(std::memory_order::acquire) == (head + 1U)) {
        if (head_.compare_exchange_weak(head, head + 1U, std::memory_order::relaxed)) break;
      } else {
